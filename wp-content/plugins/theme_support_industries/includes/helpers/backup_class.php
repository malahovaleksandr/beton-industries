...
class Bunch_Backup_class
{
	...
	function theme_options_import($file = '')
	{
		...
		$blog_page = get_page_by_title('News Grid');
		...
		$res = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."terms WHERE ".$wpdb->prefix."terms.slug = 'Main-Menu' OR ".$wpdb->prefix."terms.slug = 'Topbar-Menu' OR ".$wpdb->prefix."terms.slug = 'Services-Menu'");
		if($res){
			$nav_menu = array('');
			$nav_menu['nav_menu_locations']['main_menu'] = $res[0]->term_id;
			$nav_menu['nav_menu_locations']['topbar_menu'] = $res[1]->term_id;
