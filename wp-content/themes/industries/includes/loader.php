...
add_action( 'init', 'industries_bunch_theme_init');


if( !function_exists( 'industries_set' ) ) {
	function industries_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('industries_printr' ) ) {
	function industries_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('industries__font_awesome' ) ) {
	function industries_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = industries_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('industries_load_class' ) ) {
	function industries_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
industries_load_class( 'enqueue', 'helpers', false );
industries_load_class( 'bootstrap_walker', 'helpers', false );


if( industries_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function industries_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include INDUSTRIES_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include INDUSTRIES_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = INDUSTRIES_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = INDUSTRIES_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( industries_set( $_GET, 'page' ) == 'industries'.'_option' ) 
		{
			if( industries_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'industries_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = INDUSTRIES_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'industries'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'industries'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'industries' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'industries' ), // menu label
	));
